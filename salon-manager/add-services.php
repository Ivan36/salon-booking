...
if (strlen($_SESSION['bpmsaid']==0)) {
	...
	if(isset($_POST['submit']))
	{
		...
		$query=mysqli_query($con, "insert into  tblservices(ServiceName,Cost,sid) value('$sername','$cost','$sid')");
		if ($query) {
			echo "<script>alert('Service has been added to your salon.');</script>"; 
								...
								<form method="post">
									...
									<div class="form-group"> 
										...
										<select class="form-control" required="" name="sid">
											<?php 
											$sql=mysqli_query($con,"SELECT * from tblsalons where id='$sid'");
											while ($row = mysqli_fetch_array($sql)) {

												?>	
												<option value="<?php echo $row['id'] ?>" selected><?php echo $row['salon'] ?></option>
												<?php
											}
											?>
